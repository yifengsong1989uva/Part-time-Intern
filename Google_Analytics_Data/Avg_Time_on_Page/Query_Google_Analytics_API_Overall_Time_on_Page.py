  ...
  return analytics.reports().batchGet(
      body={
        'reportRequests': [
        {
          ...
          'pageSize':10000 #the default is 1000, indicating only first 1000 rows of query results will be returned, which is not sufficient
        }]
      }
  ).execute()
...
def organize_response(response):
    ...
    time_point=sys.argv[2][5:]+"-"+sys.argv[2][:4]
    data_df.to_csv("Time_on_Company_Page_"+time_point.replace("-","_")+".csv",index=False)
    print "Time_on_Company_Page_"+time_point.replace("-","_")+".csv"
